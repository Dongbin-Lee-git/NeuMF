...
class NCF(pl.LightningModule):
    ...
    """
    def __init__(self, num_users, num_items, ratings, all_jobIds, pre_train = False ):
        super().__init__()
...
class NeuMF(pl.LightningModule):
    ...
    def __init__(self, user_num, item_num, ratings, all_jobIds, GMF_model=None, MLP_model=None, pre_train=False):
        super().__init__()
...
def run_finetune():
    ...
    trainer_neumf = pl.Trainer(max_epochs=10, gpus=1, reload_dataloaders_every_epoch=True,
                             progress_bar_refresh_rate=50, logger=False, checkpoint_callback=False)

    trainer_neumf.fit(model_neumf)
    trainer_neumf.save_checkpoint("save_model/neumf_epo10_bat256")
